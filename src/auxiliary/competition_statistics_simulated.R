...
participant_rank_long_subset <- participant_rank_long[
    portfolio %in% c("tangency (with insider info)", "rank optimization (q=1)", "rank optimization (q=20)", "baseline")
]
...
write.csv(state_table, file.path("outputs", "tables", "state_table_simulated.csv"), row.names = FALSE)
